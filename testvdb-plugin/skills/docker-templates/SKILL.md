...
```bash
# Milvus (gRPC + health check on separate ports)
export MILVUS_PORT="19530"           # gRPC API (primary interface for pymilvus)
export MILVUS_HEALTH_PORT="9091"     # REST health check / metrics
# Qdrant
...
# Emergency cleanup (container names must match docker-compose templates)
# Milvus has 3 containers; others have 1
docker rm -f testvdb-milvus-standalone testvdb-milvus-etcd testvdb-milvus-minio
docker rm -f testvdb-qdrant testvdb-weaviate testvdb-pgvector
# With session ID suffix: testvdb-{container_name}-{SESSION_ID}
```
